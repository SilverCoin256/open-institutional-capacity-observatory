...
The source snapshot is `datasets/raw/eoir_annual.csv`, registered as `eoir_workload_annual` in `datasets/metadata/source_register.csv`. It contains annual pending, received, and completion counts for 2016 through 2026. The release preserves the snapshot rather than silently replacing it with a live endpoint.
...
In the frozen series, pending matters rose from 826,488 in 2016 to a peak of 3,925,351 in 2024, a 4.749-fold increase. QAI was positive in all eight comparable periods from 2017 through 2024 and reached 1.580 in 2024. The 2025 and 2026 rows have negative QAI values in the stored snapshot; they are reported, not treated as proof that the underlying capacity problem was resolved.
